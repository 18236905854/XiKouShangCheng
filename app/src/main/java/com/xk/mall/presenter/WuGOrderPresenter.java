 ...
 * Version V1.0
 */
public class WuGOrderPresenter extends BasePresenter<WuGOrderViewImpl> {
    public WuGOrderPresenter(WuGOrderViewImpl baseView) {
        super(baseView);
    }
    ...
    public void getOrderList(String searchName, String account, int state, int createTimeFlag, String orderAmountL, String orderAmountR, int page, int limit){
        addDisposable(apiServer.getWuGOrderList(searchName, account,state,createTimeFlag, orderAmountL, orderAmountR, page,limit), new BaseObserver(baseView) {
            @Override

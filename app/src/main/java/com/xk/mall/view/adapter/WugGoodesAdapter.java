 ...
 */
public class WugGoodesAdapter extends  RvAdapter<ActiveSectionGoodsBean>{
    ...
    public WugGoodesAdapter(Context context, List<ActiveSectionGoodsBean> list, RvListener listener) {
    ...
    private class GoodsOneHolder extends RvHolder<ActiveSectionGoodsBean>{
        ...
        @Override
        public void bindHolder(ActiveSectionGoodsBean bean, int position) {
            ...
            tv_coupom.setText("赠券"+ PriceUtil.divideCoupon(bean.getCouponValue()));
            if(bean.getStock() <= 0){

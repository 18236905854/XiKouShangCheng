...
import com.xk.mall.presenter.ZeroXiaDanPresenter;
 ...
 * 0 元拍 下单
 */
public class ZeroXiaDanActivity extends BaseActivity<ZeroXiaDanPresenter> implements ZeroXiaDanViewImpl {
    ...
    private ZeroOrderRequestBody requestBody;
    ...
    @Override
    protected ZeroXiaDanPresenter createPresenter() {
        return new ZeroXiaDanPresenter(this);
    }
    ...
    protected void initData() {
        requestBody = (ZeroOrderRequestBody) getIntent().getSerializableExtra("request_entity");
        int price = getIntent().getIntExtra("unit_price", 0);
        postage=getIntent().getIntExtra("postage",0);
        tvTotalMoney.setText(String.valueOf(PriceUtil.dividePrice(requestBody.getOrderAmount())));
    ...
    public void onClick(View view) {
        switch (view.getId()) {
            ...
            case R.id.rl_zero_xiadan_remarks:
    ...
    public void onGetAddressListSuc(BaseModel<List<AddressBean>> addressBeanList) {
        ...
        if (list.size() == 0) {
            ...
            if(addressId == null || TextUtils.isEmpty(addressId)){
                ...
                tvReceiverName.setText(bean.consigneeName);
                tvReceiverPhone.setText(bean.consigneeMobile);
                if(addressId != null && !bean.outRange){
                    //显示超出配送范围
                    isOutRange = true;
                    llAddressOutRange.setVisibility(View.VISIBLE);
                }
...
//        ToastUtils.showShortToast(mContext,"Commit Success");
        if(baseModel!=null){

package com.xk.mall.view.widget.item;
    ...
    public void onDraw(Canvas c, RecyclerView parent, RecyclerView.State state) {
        ...
        for (int i = 0; i < count; i++) {
            ...
            if (builder.orientation == LinearLayoutManager.VERTICAL) {
                left = child.getLeft() - params.leftMargin + transitionX + ConvertUtils.dp2px(builder.leftMargin);
                top = child.getBottom() + params.bottomMargin;
                right = child.getRight() + params.rightMargin - ConvertUtils.dp2px(builder.rightMargin);
                bottom = top + builder.dividerHeight + transitionY;
            } else {
                top = child.getTop() - params.topMargin + ConvertUtils.dp2px(builder.topMargin);
                bottom = child.getBottom() + params.bottomMargin + transitionY - ConvertUtils.dp2px( builder.bottomMargin);

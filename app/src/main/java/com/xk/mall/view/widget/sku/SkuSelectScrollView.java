...
//    private List<SkuListBean> skuList;
     ...
     */
    private Map<String, List<String>> getSkuGroupByName(List<GoodsSkuListBean2> list) {
        Map<String, List<String>> dataMap = new LinkedHashMap<>();
        for (GoodsSkuListBean2 sku : list) {
    ...
    private void optionLayoutEnableStatusSingleProperty() {
        ...
        for (int i = 0; i < skuList.size(); i++) {
            // 属性值是否可点击flag
            GoodsSkuListBean2 sku = skuList.get(i);
    ...
    private void optionLayoutEnableStatusMultipleProperties() {
        for (int i = 0; i < skuContainerLayout.getChildCount(); i++) {
            ...
            for (int j = 0; j < skuList.size(); j++) {
                // 属性值是否可点击flag
                boolean flag = false;
                GoodsSkuListBean2 sku = skuList.get(j);
                ...
                // flag 为false时，可点击
                if (!flag && i < attributeBeanList.size()) {//添加长度判断，避免数组越界
    ...
    public GoodsSkuListBean2 getSelectedSku() {
        ...
        for (GoodsSkuListBean2 sku : skuList) {
     ...
     * @param sku
     */
    public void setSelectedSku(GoodsSkuListBean2 sku) {
        selectedAttributeList.clear();
        for (SkuAttribute attribute : sku.getSpecMap()) {
            selectedAttributeList.add(new SkuAttribute(attribute.getSpecName(), attribute.getSpecValue()));
        }
